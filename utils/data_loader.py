...
def carregar_dados_docentes():
    return carregar_csv(r"data\processed\pesquisa\DOCENTES-ATIVOS\docentes_ativos.csv")

@st.cache_data
def carregar_dados_grupos_pesquisa():
    return carregar_csv(r"data\processed\pesquisa\GRUPO-PESQUISA\grupos_pesquisa.csv")

@st.cache_data
def carregar_dados_apq():
    return carregar_csv(r"data\processed\pesquisa\PLANILHAS-APQ\prestacao_contas_apq.csv")

@st.cache_data
def carregar_dados_icti():
    return carregar_csv(r"data\processed\pesquisa\PLANILHAS-ICTI\monitoramento_icti.csv")

@st.cache_data
def carregar_dados_bolsistas():
    pasta = os.path.join(BASE_DIR, r"data\processed\pesquisa\BOLSAS-PQ-DT")
